...
@dp.message_handler(lambda message: get_user_state(message.from_user.id) == get_state_by_key('S_REPAYMENT_OF_LOANS'))
async def repayment_of_loans(message: types.Message):
    try:
        ...
        if message.text == get_dict('back', d):
            ...
            set_user_state(user_id, get_state_by_key('S_REPAYMENT_OF_LOANS_HAMKORBANK'))
...
@dp.message_handler(lambda message: get_user_state(message.from_user.id) == get_state_by_key('S_REPAYMENT_OF_LOANS_HAMKORBANK'))
async def repayment_of_loans_hamkorbank_id(message: types.Message):
    try:
        ...
        if message.text == get_dict('cancel', d):
            ...
            set_user_state(user_id, get_state_by_key('S_REPAYMENT_OF_LOANS_PASSWORD'))
        else:
...
@dp.message_handler(lambda message: get_user_state(message.from_user.id) == get_state_by_key('S_REPAYMENT_OF_LOANS_PASSWORD'))
