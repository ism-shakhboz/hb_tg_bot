...
@dp.message_handler(lambda message: get_user_state(message.from_user.id) == get_state_by_key('S_START'))
...
@dp.message_handler(lambda message: get_user_state(message.from_user.id) == get_state_by_key('S_CONFIRM_NUMBER'))
    ...
    try:
        ...
        if message.text == get_dict('individual', d):
            ...
            if markups.buttons(d, 'feedback')["keyboard"][0][0] is None or markups.buttons(d, 'feedback')["keyboard"][0][0]=='':
                ...
                set_user_state(user_id, get_state_by_key('S_FEEDBACK'))
        else:
